...
if ending_screen=true {
	...
	if ending_static_timer>0 {
		...
		draw_set_font(fn_m3x6);
		draw_set_halign(fa_right);
		if secret_code=false { var secret_message="PHOENIX: 35.176861, 158.977194"; }
		else { var secret_message="NANAKOROBI YAOKI"; }
		sc_drawtext(screen_main_x+cam_w-3,screen_main_y+cam_h-14,secret_message,global.color_white,global.color_black,0.75,0.25,0,-1);
	}
}
//————————————————————————————————————————————————————————————————————————————————————————————————————
if fade_black>0 {
